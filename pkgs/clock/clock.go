...
func NewClock(s *settings.Settings) *Clock {
	configs = *s
	return &Clock{
...
func (c *Clock) StartSession(description string) int {
	...
	return getLimitTimeByDescription(description, configs)
}

func (c *Clock) GetCurrentCycle() string {
	cycleCount := len(configs.Cycle)
	currentSession := len(c.Sessions)
	round := currentSession / cycleCount
	return configs.Cycle[(currentSession - (round * cycleCount))]
}
...
func getLimitTimeByDescription(description string, s settings.Settings) int {
	switch description {
	case "Work":
